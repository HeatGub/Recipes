        ...
        {steps.map((step, index) => (
          <li key={step.position} className="rounded-br-2xl border-b border-(--border-muted)! px-2 sm:px-0 py-3 sm:py-4 lg:py-6">
            ...
            <span className="text-lg ml-3">
              {step.title && <span className="mr-3 text-lg text-(--accent-primary)">{l(step.title)}</span>}
            </span>
            <span className="leading-relaxed text-(--text-secondary)">{l(step.description)}</span>
          </li>
        ))}
      </ol>
    </section>
